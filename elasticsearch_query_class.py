...
class elQuery:
    def __init__(self, query_file):
        ...
        self.query_body = self.render_j2(q, **self.range)
    ...
    def query(self):
        query_body, limit = self.query_body, int(self.config['limit'])
        self.res = self.es.search(index=self.index, body=query_body)
        ip_list = self.res['aggregations']['ip']['buckets']
        return list(self.process_resp(ip_list, limit))
